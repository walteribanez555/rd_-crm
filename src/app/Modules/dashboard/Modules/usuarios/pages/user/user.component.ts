...
export class UserComponent implements OnInit {
      ...
      this.userService
        ...
        .pipe(
          switchMap((resp) => {
            ...
            const requests: any[] = this.user.rol_id
              .split(',')
              .map((rol) => this.rolService.getById(rol));
          ...
          switchMap((resp: Rol[][]) => {
            ...
            //   )
            // );
            return this.oficinasService.getAll().pipe(
              map((resp) => {
                return resp.filter( ofi => this.user?.office_id.split(',').includes(ofi.office_id.toString()));
              })
            );
          })
        )
        .subscribe({
          next: (resp: Oficina[]) => {
            ...
            this.userForm = new FormGroup({
              ...
              user_type: new FormControl(this.user!.user_type, [
                Validators.required,
              ]),
  ...
  submitForm() {
    this.userForm?.get('office_id')?.setValue(
      '' +
        this.oficinasSelected
          .map((ofi) => {
            return ofi.office_id;
          })
          .join(',')
    );
    this.userForm?.get('rol_id')?.setValue(
      '' +
        this.rolesSelectd
          .map((rol) => {
            return rol.rol_id;
    ...
    this.userService
      .updateUser(this.user!.username, this.userForm?.value)
      .subscribe({
        next: (resp) => {
          process.complete();
