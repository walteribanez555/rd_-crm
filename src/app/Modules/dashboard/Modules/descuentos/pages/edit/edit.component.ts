...
import {
  Descuento,
  DescuentoToPost,
} from 'src/app/Modules/core/models/Descuento.model';
...
export class EditDescuentoComponent implements OnInit {
      ...
      this.descuentosService
        ...
        .subscribe((descuento) => {
          ...
          (this.cantidad = new FormControl(
            ...
            (this.descuento_id = new FormControl(
              descuento[0].descuento_id,
              Validators.required
            )),
            (this.fecha_desde = new FormControl(
              descuento[0].fecha_desde
                .split('T')[0]
                .split('-')
                .reverse()
                .join('-'),
  ...
  onUpdateDescuento() {
    ...
    const status = this.descuentos ? this.descuentos[0].status : 1;
  ...
  onUpdateState(state: number) {
    ...
    this.descuentosService
      .update(this.id_descuento!, { status: state })
      .subscribe({
        next: (resp) => {
          process.complete();
          this.onSuccess('Descuento Actualizado Correctamente');
        },
        error: (err) => {
          process.complete();
          this.onError(err);
